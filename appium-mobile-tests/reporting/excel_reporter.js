...
class AppiumExcelReporter {
    ...
    async generateReport(outputPath = config.EXCEL_REPORT_PATH) {
        ...
        workbook.creator = 'Appium Mobile E2E Test Engine';
        ...
        titleCell.value = '📱 Aero-Navigator Mobile App - Appium E2E Test Execution Summary Report';
        ...
        subCell.value = `Execution Timestamp: ${new Date().toLocaleString()}  |  Platform: Android (UiAutomator2)  |  Target: com.simats.aero_navigator`;
        ...
        kpiHeader.value = '📊 Mobile E2E Key Performance Indicators (KPIs)';
        ...
        const detailSheet = workbook.addWorksheet('Detailed Test Results', { views: [{ showGridLines: true }] });
        ...
        const analyticsSheet = workbook.addWorksheet('Module Analytics', { views: [{ showGridLines: true }] });

        analyticsSheet.columns = [
            { header: 'Mobile Module Name', key: 'module', width: 38 },
        ...
        config.MOBILE_MODULES.forEach(mod => {
            ...
            analyticsSheet.addRow({ module: mod, total, passed, failed, rate });
        });

        await workbook.xlsx.writeFile(outputPath);
        console.log(`\n✅ Appium Mobile Excel Analysis Report successfully generated at:\n   ${outputPath}\n`);
        return outputPath;
    }
}
